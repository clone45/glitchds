...
void clear_ca(bool mat[][GRID_HEIGHT]) //Sets matrix to all dead
{
   ...
   int y;
    ...
    {
 	   for (x = 0; x < GRID_WIDTH; x++) mat[x][y] = 0;
    }
}
...
void clear_triggers(int triggers[][GRID_HEIGHT]) //Sets matrix to all dead
{
   ...
   int y;
    ...
    for (y = 0; y < GRID_HEIGHT; y++)
    {
 	   for (x = 0; x < GRID_WIDTH; x++) triggers[x][y] = 0;
    }
}
   ...
   int y;
     ...
     for (y = 1; y < GRID_HEIGHT-1; y++)
     {
         for (x = 1; x < GRID_WIDTH-1; x++)
         {
             ...
             //Begin counting number of neighbors:
             if (mata[x-1][y-1] == 1) neighbors += 1;
             if (mata[x-1][y] == 1) neighbors += 1;
             if (mata[x-1][y+1] == 1) neighbors += 1;
             if (mata[x][y-1] == 1) neighbors += 1;
             if (mata[x][y+1] == 1) neighbors += 1;
             if (mata[x+1][y-1] == 1) neighbors += 1;
             if (mata[x+1][y] == 1) neighbors += 1;
             if (mata[x+1][y+1] == 1) neighbors += 1;
...
void swap_ca(bool mata[][GRID_HEIGHT], bool matb[][GRID_HEIGHT]) //Replaces first matrix with second
{
   ...
   int y;
    ...
    for (y = 0; y < GRID_HEIGHT; y++)
    {
 	  for (x = 0; x < GRID_WIDTH; x++) mata[x][y] = matb[x][y];
    }
}
...
void draw_matrix(int mat[][GRID_HEIGHT], bool screen)
{
   ...
   int y;
    ...
    for (y = 1; y < GRID_HEIGHT; y++) {
 	  for (x = 1; x < GRID_WIDTH; x++) {
 		   if (mat[x][y]) draw8bitRect(x, y, pixel_width, pixel_width, screen, mat[x][y]); // int x, int y, int w, int h, bool screen, int color
 	  }
    }   
}
...
int poll_swatches()
{
   ...
   if (Stylus.Newpress)
   {
      ...
      if (y < 17)
      {
         if (x >= 112 && x <= 127) selected_swatch = 1;
         if (x >= 128 && x <= 143) selected_swatch = 2;
         if (x >= 144 && x <= 159) selected_swatch = 3;
         if (x >= 160 && x <= 175) selected_swatch = 4;
         if (x >= 176 && x <= 191) selected_swatch = 5;
         if (x >= 191 && x <= 208) selected_swatch = 6;
 		   draw_swatches(); 		  
    ...
    {
 	  selected_swatch--;
 	  if (selected_swatch < 1) selected_swatch = 6;
 	   draw_swatches(); 		  
 	   choose_swatch(selected_swatch);   
    ...
    {
 	  selected_swatch++;
 	  if (selected_swatch > 6) selected_swatch = 1;
 	   draw_swatches(); 		  
 	   choose_swatch(selected_swatch);   
...
{   
   ...
   if (old_sequencer_step == -1) old_sequencer_step = 31;
...
void draw_screen_pallet_mode()
{
    ...
    if (selected_swatch == 1) file_offset = find_position_of_filename_in_directory(swatch_0_file_assignment) - 4;
    if (selected_swatch == 2) file_offset = find_position_of_filename_in_directory(swatch_1_file_assignment) - 4;
    if (selected_swatch == 3) file_offset = find_position_of_filename_in_directory(swatch_2_file_assignment) - 4;
    if (selected_swatch == 4) file_offset = find_position_of_filename_in_directory(swatch_3_file_assignment) - 4;
    if (selected_swatch == 5) file_offset = find_position_of_filename_in_directory(swatch_4_file_assignment) - 4;
    if (selected_swatch == 6) file_offset = find_position_of_filename_in_directory(swatch_5_file_assignment) - 4;
...
void draw_screen_distortion_mode()
{
    ...
    if ((distortion_y > 20) && (distortion_y < 185)) draw8bitRectEx(0,distortion_y,255,2,0,2);
}
...
void do_interrupt()
{
   if(sequencer_status == CLOCKED) redraw_flag = 1;
}
...
int main(int argc, char** argv)
{
   ...
   // int slot_id;
    ...
    while (1)
    {
 	  ...
 	  {
 	     mode--;
 	     if (mode < 1) mode = NUMBER_OF_MODES;
 	   }
 	  ...
 	  // :: Mode Actions ::
 	   ...
 	   if (mode == SET_CLOCK_SOURCE)
 	   {
 		  ...
 		  if (sequencer_status == CLOCKED)
 		  {
 		   ...
 		   if (sequencer_status == MANUAL)
 		   {
 			  ...
 			  if(Pad.Newpress.Left)
 			  {
 			     if (stylus_y < 96) strum_quadrant = STRUM_TOP_HALF;
 			     if (stylus_y >= 96) strum_quadrant = STRUM_BOTTOM_HALF;
 			     sequencer_step = 0;
 			  ...
 			  if (stylus_y > 20)
 			  {
 				  if (old_stylus_y != stylus_y)
 				  {
 					  // clear out old vertical line
 					   if (old_stylus_y > 20) draw8bitRectEx(0,old_stylus_y,255,2,0,0);  // x,y,w,h,screen,color 				  
 	   ...
 	   if (mode == PALLET_MODE)
 	   {
 		   ...
 		   {
 			  if ((file_offset + 4) < number_of_files) file_offset ++;
 			  update_screen_pallet_mode();
 		   } 		  

 		   if (Pad.Newpress.Up)
 		   {
 			  if ((file_offset + 4) > 0) file_offset--;
 			  update_screen_pallet_mode();
 		   }
 		  
 		   if (!Pad.Held.Down && !Pad.Held.Up) key_hold_time = 0;
 	   ...
 	   if (mode == SAVE_LOAD_MODE)
 	   {
 		   ...
 		   if (Pad.Newpress.Up || Pad.Newpress.Down)
 		   {
 			   ...
 			   if (Pad.Newpress.Up) file_id++;   
 			   if (Pad.Newpress.Down) file_id--;   
 			  
 			   if (file_id > 19) file_id = 0;
 			   if (file_id < 0) file_id = 19;
...
void load_sound_from_file(int slot_id, char* filename)
{
   ...
   old_buffer = sound_slot[slot_id].data;
    ...
    if (slot_id == 0) { swatch_0_file_assignment[0]='\0'; strcpy(swatch_0_file_assignment,tmp_filename); }
    if (slot_id == 1) { swatch_1_file_assignment[0]='\0'; strcpy(swatch_1_file_assignment,tmp_filename); }
    if (slot_id == 2) { swatch_2_file_assignment[0]='\0'; strcpy(swatch_2_file_assignment,tmp_filename); }
    if (slot_id == 3) { swatch_3_file_assignment[0]='\0'; strcpy(swatch_3_file_assignment,tmp_filename); }
    if (slot_id == 4) { swatch_4_file_assignment[0]='\0'; strcpy(swatch_4_file_assignment,tmp_filename); }
    if (slot_id == 5) { swatch_5_file_assignment[0]='\0'; strcpy(swatch_5_file_assignment,tmp_filename); }
